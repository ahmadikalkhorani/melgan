...
class AudioVisualModel(pl.LightningModule):
    def __init__(
        ...
        v = 1 if args.audio_only else 1
        netG = Generator(args.n_mel_channels * v, args.ngf, args.n_residual_layers)
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--noise_db_bounds", type=str2list, default='(-15, -10)')
        parser.add_argument("--noise_db_bounds_test", type=str2list, default='(-15, -10)')
    ...
    def forward(self, batch_inp):
        ...
        x_pred_t = self.netG(self.v_front({"mel_noisy": self.fft(batch_inp["audio"][:, -1, ...]), "video": v_t}))
    ...
    def validation_step(self, batch_inp, batch_idx):
        ...
        losses = {}
        self.post_process(x_pred_t, x_t, s_t, s_pred_t, mel_v, losses, trainval = "val")
